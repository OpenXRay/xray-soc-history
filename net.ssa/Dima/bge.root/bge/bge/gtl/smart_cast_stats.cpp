...
{
	if (m_stats.empty()) {
		ui().log						("CONGRATULATIONS : SmartCast stats is empty!!!");
		return;
	}
	...
	ui().log							("SmartCast stats (different %d, total %d) : ",(u32)m_stats.size(),total);

	I									= m_temp.begin();
	for ( ; I != E; ++I)
		ui().log						("%8d %6.2f% : smart_cast<%s>(%s)",(*I).m_count,float((*I).m_count)*100.f/float(total),(*I).m_to,(*I).m_from);
}
