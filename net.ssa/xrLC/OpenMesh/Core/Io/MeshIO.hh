...
bool read_mesh(Mesh& _mesh, const std::string& _filename, Options& _opt) 
{
  ...
  return IOManager::Instance().read(_filename, importer, _opt); 
}
...
{ 
  ...
  return IOManager::Instance().write(_filename, exporter, _opt); 
}
...
{
  ExporterT<Mesh> exporter(_mesh);
  return IOManager::Instance().binary_size(_format, exporter, _opt);
}
