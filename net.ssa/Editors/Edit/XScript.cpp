...
XScr::XScr( char *_fileName, bool _bLoad ){
	...
	if( _bLoad ){
		if( !LoadFile() ){
			Log.Msg( "Can't load '%s' !", _fileName );
			return; }
		if( !scrCompiled )
			if( !Compile() ){
				Log.Msg( "Can't compile '%s' !", _fileName );
				return; }
	}
}
...
XScr::XScr( char *_Buffer, int _bufferSize, bool _Text ){
	...
	if( _Text ){
		...
		scrCompiled = false;
		if( !Compile() ){
			Log.Msg( "Can't compile '%s' !", scrName );
			return; }
	}
	else{
		...
		if( _H->magic != sizeof(_H) ){
			Log.Msg( "Script '%s' is not valid !", scrName );
			return; };
...
bool XScr::LoadBinary(){
	...
	int hfile = open( scrName, _O_RDONLY|_O_BINARY );
	if( hfile <=0 ){
		return false;
	}	
...
bool XScr::LoadText(){
	...
	if( hfile <=0 ){
		return false; }
	scrBufferSize = filelength( hfile );
	if( scrBufferSize<=0 ){
		Log.Msg( "zero-length file: '%s'...", scrName );
		_close(hfile); return false; }
	scrBuffer = new char[scrBufferSize];
	if( scrBufferSize != _read( hfile, scrBuffer, scrBufferSize ) ){
		Log.Msg( "error reading file '%s'...", scrName );
		_close(hfile); return false; }
	_close( hfile );
	return true;
...
bool XScr::SaveBinary( char *_fileName ){
	...
	for( DWORD i=0; i<scrTokens.size(); i++){
...
bool XScr::Merge( XScr *_S ){
	...
	if( _S->scrTokens.size() <= 0 )
		return false;
	for( DWORD i=0; i<_S->scrTokens.size(); i++)
