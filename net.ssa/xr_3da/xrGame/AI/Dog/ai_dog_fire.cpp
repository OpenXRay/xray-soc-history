////////////////////////////////////////////////////////////////////////////
//	Module 		: ai_dog_fire.cpp
//	Created 	: 23.07.2002
//  Modified 	: 07.11.2002
//	Author		: Dmitriy Iassenev
//	Description : Fire and enemy parameters for monster "Dog"
////////////////////////////////////////////////////////////////////////////
...
void CAI_Dog::Exec_Action(float dt)
{
...
void CAI_Dog::HitSignal(float amount, Fvector& vLocalDir, CObject* who, s16 element)
{
	// Save event
	Fvector D;
	...
	if (g_Health() - amount <= 0) {
		if ((m_tpCurrentGlobalAnimation) && (!m_tpCurrentGlobalBlend->playing))
			if (m_tpCurrentGlobalAnimation != m_tDogAnimations.tNormal.tGlobal.tpaDeath[0])
				m_tpCurrentGlobalBlend = PKinematics(pVisual)->PlayCycle(m_tpCurrentGlobalAnimation = m_tDogAnimations.tNormal.tGlobal.tpaDeath[::Random.randI(0,2)]);
	}
}

float CAI_Dog::EnemyHeuristics(CEntity* E)
{
	if (E->g_Team()  == g_Team())	
...
void CAI_Dog::SelectEnemy(SEnemySelected& S)
{
	// Initiate process
...
float CAI_Dog::CorpHeuristics(CEntity* E)
{
	if (!E->g_Alive()) {
...
void CAI_Dog::SelectCorp(SEnemySelected& S)
{
	// Initiate process
	objVisible&	Known	= Level().Teams[g_Team()].Squads[g_Squad()].KnownEnemys;
	S.Enemy					= 0;
...
void CAI_Dog::vfUpdateMoraleBroadcast(float fValue, float fRadius)
{
