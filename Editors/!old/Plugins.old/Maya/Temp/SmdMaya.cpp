...
MStatus CXRaySkinExport::writer (const MFileObject& file, const MString& options, FileAccessMode mode)
{
	...
	if (m_fSkinCluster)
		Msg("Trying to export mesh - ",m_strSkinCluster.asChar());
...
// frame of animation
MStatus CXRaySkinExport::setUpBoneMap(void)
{
	MStatus	status;
	int lastJointID = -1;
	m_jointMap.clear();
...
/*
	// now go through only the skinned bones...
		for (int i=0; i<m_rgInfs.length(); i++)
		{
			MDagPath jointPath = m_rgInfs[i];

			SmdBone *bone = new SmdBone;
			MFnDependencyNode fnJoint = jointPath.node();
			bone->m_id = ++lastJointID;
			bone->m_name = strdup(fnJoint.name().asChar());
			bone->m_path = jointPath;

			// Add this joint to our joint map so we can look it up by name
			// cout << "Added bone '" << bone->m_name << "' to bone map\n";
			m_jointMap.insert( NameIntMap::value_type(bone->m_name, bone->m_id) );
			bone->parentId = -1;
...
{
	for (SmdTriIt  t_it=m_triList.begin();  t_it!=m_triList.end();  t_it++)	xr_delete(*t_it);
	for (SmdVertIt v_it=m_vertList.begin(); v_it!=m_vertList.end(); v_it++) xr_delete(*v_it);
	for (SmdBoneIt b_it=m_boneList.begin(); b_it!=m_boneList.end(); b_it++) xr_delete(*b_it);
	xr_delete		(m_rgWeights);
