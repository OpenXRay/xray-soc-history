...
public:
    ...
    template <class T>
    IC FlagValue<T>*	CreateFlag		(PropItemVec& items, AnsiString key, T* val, T::TYPE mask, LPCSTR c0=0, LPCSTR c1=0, u32 flags=0)
    {   return			(FlagValue<T>*)	AppendValue		(items,key,xr_new<FlagValue<T> >(val,mask,c0,c1,flags),PROP_FLAG);    }
    template <class T>
