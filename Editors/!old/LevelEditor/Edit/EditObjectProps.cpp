...
void CEditableObject::FillSurfaceProps(CSurface* SURF, LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateCaption		(items, PrepareKey(pref,"Face Count"),	ref_str().sprintf("%d",GetSurfFaceCount(SURF->_Name())));   
}
//---------------------------------------------------------------------------
...
void CEditableObject::FillBasicProps(LPCSTR pref, PropItemVec& items)
{
    ...
    V=PHelper().CreateCaption  	(items, PrepareKey(pref,"Transform\\BBox Min"),	ref_str().sprintf("{%3.2f, %3.2f, %3.2f}",VPUSH(GetBox().min)));
    V=PHelper().CreateCaption  	(items, PrepareKey(pref,"Transform\\BBox Max"),	ref_str().sprintf("{%3.2f, %3.2f, %3.2f}",VPUSH(GetBox().max)));
...
void CEditableObject::FillSummaryProps(LPCSTR pref, PropItemVec& items)
{
    ...
    PHelper().CreateRText(items,PrepareKey(pref, "Game options\\User Data"),&m_ClassScript);
}
//---------------------------------------------------------------------------
...
void CEditableObject::FillSurfaceList(LPCSTR pref, ListItemsVec& items, int modeID)
{
	...
	if (pref&&s_lst.empty()) LHelper().CreateItem(items, pref, modeID, ListItem::flSorted);
...
{
    BoneVec& b_lst 		= Bones();
	if (pref&&b_lst.empty()) LHelper().CreateItem(items, pref, modeID, ListItem::flSorted);
    for(BoneIt b_it=b_lst.begin(); b_it!=b_lst.end(); b_it++){
		...
		LHelper().CreateItem(items, PrepareKey(path.c_str(), (*b_it)->Name()).c_str(), modeID, 0, *b_it);
    }
}
...
void CEditableObject::FillMotionList(LPCSTR pref, ListItemsVec& items, int modeID)
{
	...
	if (pref&&m_lst.empty()) LHelper().CreateItem(items, pref,  modeID, ListItem::flSorted);
