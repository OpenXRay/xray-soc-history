...
*/
	...
	void Save(char *name)
	{
		...
		for (SVERTIt V=Vlist.begin(); V!=Vlist.end(); V++){
			...
			F.w_float(pV->uv[0].x); F.w_float(pV->uv[0].y);		// tu,tv
			F.w_u32(pV->bone);
		}
		F.close_chunk();
...
bool SceneBuilder::SaveObjectSkeletonOGF(const char* fn, CEditableObject* obj){
	...
	F.w_u32(obj->BoneCount());
    i=0;
    for (BoneIt b_it=obj->FirstBone(); b_it!=obj->LastBone(); b_it++,i++){
		F.w_stringZ	((*b_it)->Name());
		F.w_stringZ	(((*b_it)->ParentIndex()==-1)?"":(*b_it)->Parent());
		Fobb	obb;
