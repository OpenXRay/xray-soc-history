...
TfrmTools* TfrmTools::form=0;

//---------------------------------------------------------------------------
__fastcall TfrmTools::TfrmTools(TComponent* Owner)
    : TForm(Owner)
{
...
void __fastcall TfrmTools::OnModified()
{
}
//---------------------------------------------------------------------------
void __fastcall TfrmTools::ShowEditor()
{
	if (!form){
    	form = xr_new<TfrmTools>((TComponent*)0);
		Scene->lock();
    }
    form->Show();
}
//---------------------------------------------------------------------------
void __fastcall TfrmTools::FormShow(TObject *Sender)
{
	UI->CheckWindowPos(this);
}
//---------------------------------------------------------------------------
void __fastcall TfrmTools::FormCloseQuery(TObject *Sender,
      bool &CanClose)
{
...
void __fastcall TfrmTools::OnItemFocused(TElTreeItem* item)
{
...
void __fastcall TfrmTools::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
...
//------------------------------------------------------------------------------
bool TfrmTools::OnFrameCountAfterEdit  (PropValue* v, s32& val)
{
...
void TfrmTools::UpdateProperties()
{
    // fill data
    PropItemVec items;
	if (m_CurrentItem){
        ...
        V->OnAfterEditEvent.bind(this,&TfrmTools::OnFrameCountAfterEdit);
...
void TfrmTools::SetCurrentItem(CLAItem* I, ListItem* owner)
{
	m_CurrentItem 				= I;
...
void __fastcall TfrmTools::ebAddAnimClick(TObject *Sender)
{
    // folder name
    AnsiString pref;
    ...
    pref 						+= "la";
    AnsiString name				= FHelper.GenerateName	(pref.c_str(),2,fastdelegate::bind<TFindObjectByName>(this,&TfrmTools::FindItemByName),false,true);
...
void TfrmTools::FindItemByName(LPCSTR name, bool& res)
{	
	res = !!LALib.FindItem(name);
}

void __fastcall TfrmTools::ebCloneClick(TObject *Sender)
{
	if (m_CurrentItem){
        // folder name
	    AnsiString name			= FHelper.GenerateName	(m_CurrentItem->cName.c_str(),2,fastdelegate::bind<TFindObjectByName>(this,&TfrmTools::FindItemByName),false,true);
...
void __fastcall TfrmTools::ebDeleteAnimClick(TObject *Sender)
{
	m_Items->RemoveSelItems		();	
}
//---------------------------------------------------------------------------

void __fastcall TfrmTools::ebSaveClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebReloadClick(TObject *Sender)
{
	ebSave->Enabled 			= false;
	m_CurrentItem 				= 0;
	LALib.Reload				();
...
void __fastcall TfrmTools::pbGMouseDown(TObject *Sender,
...
void __fastcall TfrmTools::pbGMouseMove(TObject *Sender,
...
void __fastcall TfrmTools::pbGMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (iMoveKey>=0){
...
void __fastcall TfrmTools::ebDeleteKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebCreateKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::pbGPaint(TObject *Sender)
{
	TRect R 	= pbG->ClientRect;
...
void __fastcall TfrmTools::sePointerKeyDown(TObject *Sender,
...
void __fastcall TfrmTools::sePointerExit(TObject *Sender)
{
	pbG->Repaint();
}
//---------------------------------------------------------------------------

void __fastcall TfrmTools::OnIdle()
{
	if (form){
...
void __fastcall TfrmTools::sePointerChange(TObject *Sender)
{
	sePointer->Color = TColor(m_CurrentItem->IsKey(sePointer->Value)?0x00BFFFFF:0x00A0A0A0);
    UpdateProperties();
}
...
void __fastcall TfrmTools::ebFirstKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebPrevKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebNextKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebLastKeyClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebMoveKeyLeftClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebMoveKeyRightClick(TObject *Sender)
{
	if ((sePointer->Value!=0)&&(m_CurrentItem->IsKey(sePointer->Value))){
    	int f0, f1;
    	f1 = f0 = sePointer->Value;
...
void __fastcall TfrmTools::ebLastFrameClick(TObject *Sender)
{
...
void __fastcall TfrmTools::ebFirstFrameClick(TObject *Sender)
{
	sePointer->Value	= 0;
	pbG->Repaint();
}
...
void __fastcall TfrmTools::wnShapeKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
    if (m_CurrentItem){
        if (Key==VK_LEFT){
