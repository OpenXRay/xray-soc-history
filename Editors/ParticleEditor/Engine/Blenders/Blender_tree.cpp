...
void	CBlender_Tree::Compile	(CBlender_Compile& C)
{
	...
	{
	case 0:		// deffer
		if (oBlend.value)	C.r_Pass	(r2v("deffer_tree_flat"),r2p("deffer_base_aref_flat"),FALSE);
		else				C.r_Pass	(r2v("deffer_tree_flat"),r2p("deffer_base_flat"),FALSE);
		C.r_Sampler			("s_base",C.L_textures[0]);
		C.r_End				();
		break;
	case 1:		// smap-direct
		if (oBlend.value)	C.r_Pass	(r2v("shadow_direct_tree"),r2p("shadow_direct_aref"),FALSE,TRUE,TRUE,TRUE,D3DBLEND_SRCALPHA,D3DBLEND_INVSRCALPHA,TRUE,220);
		else				C.r_Pass	(r2v("shadow_direct_base"),r2p("shadow_direct_base"),FALSE);
		C.r_Sampler			("s_base",C.L_textures[0]);
		C.r_End				();
		break;
	case 2:		// smap-point
		if (oBlend.value)	C.r_Pass	(r2v("shadow_point_aref"),r2p("shadow_point_aref"),FALSE);
		else				C.r_Pass	(r2v("shadow_point_base"),r2p("shadow_point_base"),FALSE);
		C.r_Sampler			("s_base",C.L_textures[0]);
