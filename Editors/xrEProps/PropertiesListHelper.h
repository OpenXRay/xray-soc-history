...
public:
    ...
    virtual ChooseValue*		__stdcall	CreateChoose	    (PropItemVec& items, ref_str key, ref_str* val, u32 mode, LPCSTR path=0);
	...
	virtual RToken8Value* 		__stdcall	CreateRToken8	    (PropItemVec& items, ref_str key, u8* val, RTokenVec* token);
	virtual RToken16Value* 		__stdcall	CreateRToken16	    (PropItemVec& items, ref_str key, u16* val, RTokenVec* token);
	virtual RToken32Value* 		__stdcall	CreateRToken32	    (PropItemVec& items, ref_str key, u32* val, RTokenVec* token);
	virtual TokenValueSH*   	__stdcall	CreateTokenSH	    (PropItemVec& items, ref_str key, u32* val, u32 cnt, const TokenValueSH::Item* lst);
	virtual ListValue* 	 		__stdcall	CreateList		    (PropItemVec& items, ref_str key, ref_str* val, RStringVec* lst);
    ...
    virtual ref_str  			__stdcall	PrepareKey			(LPCSTR pref0,	LPCSTR pref1, 	LPCSTR pref2,	LPCSTR key);
};
//---------------------------------------------------------------------------
#endif
