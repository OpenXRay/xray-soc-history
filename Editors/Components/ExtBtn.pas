...
begin
  ...
  try
    ...
    TmpImage.Canvas.Brush.Color := FBtnColor;
...
begin
  ...
  with GlyphPos do
    if Transparent or (State = bsExclusive) then
      ...
      ImageList_DrawEx(FGlyphList.Handle, Index, Canvas.Handle, X, Y, 0, 0,
        ColorToRGB(FBtnColor), clNone, ILD_Normal);   
end;
...
begin
	...
	if not FFlat then begin
  	...
  	if FState in [bsDown, bsExclusive] then  begin
    	if ( not FTransparent and (FState in [bsExclusive]) and (not FFlat or not FMouseInControl or FHotTrack)) then begin
      		Canvas.Brush.Bitmap := AllocPatternBitmap(FBtnColor, FDownColor);
        	if (FBevelShow or FHotTrack) then Canvas.FillRect(PaintRect);
    	end;
    	if (FBevelShow or FHotTrack) then begin
      		Offset.X := 1;
      		Offset.Y := 1;
    	end else begin
      		Offset.X := 0;
...
procedure TExtBtn.SetBtnColor(Value: TColor);
begin
...
procedure TExtBtn.SetDownColor(Value: TColor);
begin
