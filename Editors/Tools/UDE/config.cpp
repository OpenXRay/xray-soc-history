...
LPSTR vsConfig::FindParam (list<LPSTR>::iterator i, LPCSTR pParam)
{
    ...
    for (list<LPSTR>::iterator a = i; a != m_pStrings->end (); ++a)
        ...
        LPSTR str = *a;
        if (*str == '[') return NULL;
        char buf[64], *eol = strtok (str, " \t");
        if (!eol) continue;
        //int ilen = (int)(eol - str);
        //memcpy (buf, str, ilen);
        //buf[ilen] = 0;
        if (!stricmp (pParam, eol)) return str;
        }
    return NULL;
}
//---------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------
int                 vsConfig::GetUnitsCount       (LPCSTR lpSection)
{
    list<LPSTR>::iterator i = FindSection (lpSection);
    if (i == m_pStrings->end ()) return 0;
    int count = 0;
    ++i;
    for (; i != m_pStrings->end (); ++i)
        {
        LPSTR s = *i;
        if (*s == '[') break;
        if (*s == ';' || *s == ' ' || *s == '#') continue;
...
//---------------------------------------------------------------------------------------
void                vsConfig::EnumerateItems      (LPCSTR lpSection, int index, LPSTR lpParam, LPSTR lpValue)
{
    list<LPSTR>::iterator i = FindSection (lpSection);
    if (i == m_pStrings->end ())
       {
    ...
    for (; i != m_pStrings->end (); ++i)
        ...
        if (count == index)
           {
           SplitParamValue (s, lpParam, lpValue);
           return;
           }
        count++;
        }
...
//---------------------------------------------------------------------------------------
void                vsConfig::SetParam            (LPCSTR pSection, LPCSTR pParam)
{
    if (!m_pStrings) m_pStrings = new list<LPSTR> ();
    if (!m_pStrings) return;
    if (!pSection || !*pSection || !pParam || !*pParam) return;

    list<LPSTR>::iterator isection = FindSection (pSection);
    if (isection == m_pStrings->end ())
       {
       char *str = new char [strlen (pSection) + 4];
       if (!str) return;
       sprintf (str, "[%s]", pSection);
       m_pStrings->push_back (str);
       isection = FindSection (pSection);
       if (isection == NULL) return;
       str = new char [strlen (pParam) + 4];
       if (!str) return;
       strcpy (str, pParam);
       m_pStrings->insert (++isection, str);
       return;
