...
*/
			...
			for (int i=0; i < cTri; i++) {
				...
				if (iTexture<0)
					Debug.fatal("Can't find material for polygon: %d",meshPoly.index());
...
LPCSTR CXRayObjectExport::getMaterialName(MDagPath & mdagPath, int cid, int objectIdx)
{
	...
	if (!materialsEqual){
		...
		if (mLength==0) Debug.fatal("Object '%s' has polygon '%d' without material.",0,cid);
		if (mLength>1){
			Debug.fatal("Object '%s' has polygon '%d' with more than one material.",0,cid);
		}
	}else{
