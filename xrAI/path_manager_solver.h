...
{
protected:
	typedef CProblemSolver<T1,T2,T3,T4,T5>	_Graph;
...
public:
	...
	IC		const _index_type	&get_value		(const_iterator &i, bool reverse_search = false) const;
#else
	IC		const _index_type	&get_value		(const_iterator &i, bool reverse_search = true) const;
#endif
