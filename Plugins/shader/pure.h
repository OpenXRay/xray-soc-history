...
DECLARE_MESSAGE(Render2D);
DECLARE_MESSAGE(DeviceDestroy);
DECLARE_MESSAGE(DeviceCreate);
DECLARE_MESSAGE(NetworkMessage);
DECLARE_MESSAGE(SoundMove);
DECLARE_MESSAGE(AppActivate);
DECLARE_MESSAGE(AppDeactivate);
DECLARE_MESSAGE(GameStateChange);
DECLARE_MESSAGE(CameraChange);
...
template <class ENGINE_API T> class ENGINE_API CRegistrator		// the registrator itself
{
...
public:
	...
	void Add	(T *obj, int priority=REG_PRIORITY_NORMAL, DWORD flags=0)
	{
		...
		R[Count].Flags	=flags;
		Count++;
		VERIFY(Count<MAX_REGISTERED_OBJECTS);
		Resort	( );
	};
	void Remove	(T *obj)
	{
