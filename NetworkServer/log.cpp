...
void InterpretError(long h, char const *p, int l) 
{
	THROW;
}

extern bool bMustExit;

static const char *	logFName	= "netsrv.log";
...
{
	if (logWindow) {
		HWND S = GetDlgItem( logWindow, IDC_PING );
...
void Timestamp(DWORD t) 
{
	if (logWindow) {
		char buf[256];
		sprintf(buf,"Local timestamp: %d",t);
		HWND S = GetDlgItem( logWindow, IDC_TIMESTAMP );
		SetWindowText(S,buf);
	}
}
