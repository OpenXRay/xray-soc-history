...
class CSkeletonCollectorPacked
{
    ...
    DWORD			VPack(SSkelVert& V){
		...
		DWORD ix,iy,iz;
		ix = iFloor(float(V.P.x-VMmin.x)/VMscale.x*clpMX);
		iy = iFloor(float(V.P.y-VMmin.y)/VMscale.y*clpMY);
		iz = iFloor(float(V.P.z-VMmin.z)/VMscale.z*clpMZ);
		R_ASSERT(ix<=clpMX && iy<=clpMY && iz<=clpMZ);

		{
			DWORDVec& vl=VM[ix][iy][iz];
		...
		if (0xffffffff==P)
		{
			...
			DWORD ixE,iyE,izE;
			ixE = iFloor(float(V.P.x+VMeps.x-VMmin.x)/VMscale.x*clpMX);
			iyE = iFloor(float(V.P.y+VMeps.y-VMmin.y)/VMscale.y*clpMY);
			izE = iFloor(float(V.P.z+VMeps.z-VMmin.z)/VMscale.z*clpMZ);
...
public:
    ...
    SkelVertVec& 	getV_Vec(){return verts;}
    SSkelVert*		getV()	{ return verts.begin();	}
    int				getVS()	{ return verts.size();	}
    SSkelFace*		getT()	{ return faces.begin();	}
    int				getTS() { return faces.size();	}
};
//----------------------------------------------------
class CExportSkeletonOGF: public CSkeletonCollectorPacked{
...
//----------------------------------------------------
public:
			CExportSkeletonOGF	(CEditableObject* object){m_Source=object;}
    bool    Export				(LPCSTR fn){
        vector<FvectorVec>	bone_points;
        ResetAnimation();
        if( MeshCount() == 0 ) return false;

        R_ASSERT(IsDynamic()&&IsSkeleton());
        ...
        for(EditMeshIt m_it=m_Meshes.begin();m_it!=m_Meshes.end();m_it++){
            CEditableMesh* M = *m_it;
            // generate vertex offset
            if (!(M->m_LoadState&EMESH_LS_SVERTICES)) M->GenerateSVertices();
            // generate normals
            if (!(M->m_LoadState&EMESH_LS_FNORMALS)) M->GenerateFNormals();
            vnormals.clear();
            Fvector N;
            for(FvectorIt pt=M->m_Points.begin();pt!=M->m_Points.end();pt++){
                N.set(0,0,0);
                INTVec& a_lst = M->m_Adjs[pt-M->m_Points.begin()];
                VERIFY(a_lst.size());
                for (INTIt i_it=a_lst.begin(); i_it!=a_lst.end(); i_it++)
                    N.add(M->m_FNormals[*i_it]);
                N.normalize_safe();
                vnormals.push_back(N);
            }
            // fill faces
            for (SurfFacesPairIt sp_it=M->m_SurfFaces.begin(); sp_it!=M->m_SurfFaces.end(); sp_it++){
                INTVec& face_lst = sp_it->second;
                CSurface* surf = sp_it->first;
                DWORD dwTexCnt = ((surf->_FVF()&D3DFVF_TEXCOUNT_MASK)>>D3DFVF_TEXCOUNT_SHIFT);
                R_ASSERT(dwTexCnt==1);
                for (INTIt f_it=face_lst.begin(); f_it!=face_lst.end(); f_it++){
                    st_Face& face = M->m_Faces[*f_it];
                    {
                        SSkelVert v[3];
                        for (int k=0; k<3; k++){
                            st_FaceVert& 	fv = face.pv[k];
                            st_SVert& 		sv = M->m_SVertices[fv.pindex];
                            int offs = 0;
                            Fvector2* 		uv = 0;
                            for (DWORD t=0; t<dwTexCnt; t++){
                                st_VMapPt& vm_pt 	= M->m_VMRefs[fv.vmref][t+offs];
                                st_VMap& vmap		= M->m_VMaps[vm_pt.vmap_index];
                                if (vmap.type!=vmtUV){ offs++; t--; continue; }
                                uv					= &vmap.getUV(vm_pt.index);
                            }
                            R_ASSERT2(uv,"uv empty");
                            v[k].set(M->m_Points[fv.pindex],sv.offs,sv.norm,*uv,sv.bone);
                        }
                        add_face(v[0], v[1], v[2]);
    ...
    //S	l_faces_cnt		+= GetFaceCount();
    //S	l_vertices_cnt  += GetVertexCount();
    //S	l_faces			= new b_face[l_faces_cnt];
    //S	l_vertices		= new b_vertex[l_vertices_cnt];
        for(EditMeshIt M=m_Meshes.begin();M!=m_Meshes.end();M++){
            // save offset
            int point_offs=l_vertices_it;
            // fill vertices
    ...
    //                    ELog.Msg(mtInformation,"#%d - HPB: %3.2f, %3.2f, %3.2f",frm,-R.x,-R.y,R.z);
    //                    ELog.Msg(mtInformation,"#%d - PARENT: [%3.2f, %3.2f, %3.2f, %3.2f], [%3.2f, %3.2f, %3.2f, %3.2f], [%3.2f, %3.2f, %3.2f, %3.2f], [%3.2f, %3.2f, %3.2f, %3.2f]",frm,
...
/*
        ...
        void	MakeProgressive(){
            if (Flist.size()>1) {
                ...
                // Convert
                PM_Result R;
                I_Current = -1;//PM_Convert(Indices.begin(),Indices.size(), &R);
