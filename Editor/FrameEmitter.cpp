...
void TfraEmitter::GetInfoFirst(const PS::SEmitterDef& E){
    ...
    ebBirthFunc->Down				= E.m_dwFlag&PS_EM_BIRTHFUNC;
    seParticleLimit->ObjFirstInit 	(E.m_ParticleLimit);
	// burst
	cbBurst->ObjFirstInit			((E.m_dwFlag&PS_EM_BURST)?cbChecked:cbUnchecked);
    // play once
    cbPlayOnce->ObjFirstInit		((E.m_dwFlag&PS_EM_PLAY_ONCE)?cbChecked:cbUnchecked);
}
//---------------------------------------------------------------------------
...
void TfraEmitter::GetInfoNext(const PS::SEmitterDef& E){
	...
	// burst
	cbBurst->ObjNextInit			((E.m_dwFlag&PS_EM_BURST)?cbChecked:cbUnchecked);
    // play once
    cbPlayOnce->ObjNextInit			((E.m_dwFlag&PS_EM_PLAY_ONCE)?cbChecked:cbUnchecked);
}
//---------------------------------------------------------------------------
...
void TfraEmitter::SetInfo(PS::SEmitterDef& E){
	...
	// cone
    E.m_dwFlag						= 0|(ebBirthFunc->Down?PS_EM_BIRTHFUNC:0);
	...
	// burst
    E.m_dwFlag						|= cbBurst->Checked?PS_EM_BURST:0;
    // play once
    E.m_dwFlag						|= cbPlayOnce->Checked?PS_EM_PLAY_ONCE:0;
}
//---------------------------------------------------------------------------
