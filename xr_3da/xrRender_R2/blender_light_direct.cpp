...
void	CBlender_accum_direct::Compile(CBlender_Compile& C)
{
	...
	if (RImplementation.b_nv3x)
	{
		...
		{
		case 0:		// near pass
			C.r2_Pass				("null",		r2p("r2_accum_direct_1"),TRUE,FALSE,FALSE);
			...
			break;
		case 1:		// far pass
			C.r2_Pass				("null",		r2p("r2_accum_direct_2"),TRUE,FALSE,TRUE,D3DBLEND_ONE,D3DBLEND_ONE);
			C.r2_Sampler_rtf		("s_position",	r2_RT_P);
		...
		case 2:		// lighting pass
			C.r2_Pass				("null",		r2p("r2_accum_direct_L"),FALSE,FALSE,TRUE,	D3DBLEND_DESTCOLOR,	D3DBLEND_ZERO);
			C.r2_Sampler_rtf		("s_position",	r2_RT_P);
