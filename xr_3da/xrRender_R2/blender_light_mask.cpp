...
void	CBlender_accum_direct_mask::Compile(CBlender_Compile& C)
{
	...
	case 0:	// white mask for skybox
		C.r_Pass			("null",			r2p("bloom_mask"),FALSE,FALSE);
		C.r_End				();
		break;
	case 1:	// mask downsample
		C.r_Pass			("null",			r2p("bloom_mask_filter"),FALSE,FALSE);
		C.r_Sampler_rtf		("s_accumulator",	r2_RT_accum);
		C.r_End				();
		break;
	case 2:	// stencil mask for directional light
		C.r_Pass			("null",			r2p("accum_direct_mask"),FALSE,FALSE,TRUE,D3DBLEND_ZERO,D3DBLEND_ONE,TRUE,4);
