	...
	case 0: 	// deffer
		{
			...
			{
				// bump found
				C.r_Pass		(r2v("deffer_base_bump"),r2p("deffer_base_aref_bump"),FALSE);
				...
				// flat
				C.r_Pass		(r2v("deffer_base_flat"),r2p("deffer_base_aref_flat"),FALSE);
				C.r_Sampler		("s_base",C.L_textures[0]);
				C.r_End			();
			}
		}
		break;
	case 1:		// smap-direct
		if (RImplementation.b_nv3x)	C.r_Pass	(r2v("shadow_direct_aref"),r2p("shadow_direct_aref"),FALSE,TRUE,TRUE,TRUE,D3DBLEND_SRCALPHA,D3DBLEND_INVSRCALPHA,TRUE,220);
		else						C.r_Pass	(r2v("shadow_direct_aref"),r2p("shadow_direct_aref"),FALSE);
		C.r_Sampler		("s_base",C.L_textures[0]);
		C.r_End			();
		break;
	case 2:		// smap-point
