...
void CPatrolPathManager::select_point(const Fvector &position, u32 &dest_vertex_id)
{
	...
	u32						temp = u32(-1);
	if (!actual() || (m_curr_point_index >= m_path->tpaWayPoints.size()) || (position.distance_to_sqr(m_path->tpaWayPoints[m_curr_point_index].tWayPoint) > 1.f)) {
		switch (m_start_type) {
	...
	if (m_callback)
		if (m_callback->m_lua_object)
			...
			if (m_callback->m_lua_function)
				(*m_callback->m_lua_function)(CLuaGameObject(dynamic_cast<CGameObject*>(this)),u32(CScriptMonster::eActionTypeMovement),m_curr_point_index);
