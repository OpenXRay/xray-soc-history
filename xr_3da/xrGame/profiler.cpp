...
IC	ref_str CProfiler::convert_string(LPCSTR str)
{
	string256	temp;
	LPCSTR		i, j = str;
	u32			count = 0;
	...
	for (u32 k = 0; k<count; ++k)
		strcat	(temp,indent);
	strcat		(temp,j);
	count		= xr_strlen(temp);
	for ( ; count < m_max_string_size; ++count)
		temp[count] = white_character;
	temp[m_max_string_size] = 0;
	return		(ref_str(temp));
}
...
void CProfiler::setup_timer	(LPCSTR timer_id, u64 timer_time)
{
	...
	if (i == m_timers.end()) {
		i				= m_timers.insert(std::make_pair(timer_id,CProfileStats())).first;
		m_actual		= false;
	}
...
void CProfiler::show_stats	(CGameFont *game_font, bool show)
{
	...
	if (!m_actual) {
		...
		for ( ; I != E; ++I)
			m_max_string_size	= _max(m_max_string_size,compute_string_length((*I).first));
	}
	m_actual					= true;
	{
		TIMERS::iterator		I = m_timers.begin();
		TIMERS::iterator		E = m_timers.end();
		for ( ; I != E; ++I)
			game_font->OutNext	("%s.. %.3f",*convert_string((*I).first),(*I).second.m_time);
	}
}
