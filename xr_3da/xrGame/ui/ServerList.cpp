...
void CServerList::Init(int x, int y, int width, int height){
...
void CServerList::InitFromXml(CUIXml& xml_doc, const char* path){
	...
	m_itemInfo.size.icon	= xml_doc.ReadAttribInt( strconcat(buf, path, ":sizes"), 0, "icon");
	m_itemInfo.size.server	= xml_doc.ReadAttribInt( buf, 0, "server");
	m_itemInfo.size.map		= xml_doc.ReadAttribInt( buf, 0, "map");
	m_itemInfo.size.game	= xml_doc.ReadAttribInt( buf, 0, "game");
	m_itemInfo.size.players	= xml_doc.ReadAttribInt( buf, 0, "players");
	m_itemInfo.size.ping	= xml_doc.ReadAttribInt( buf, 0, "ping");
////////////////////////////////////////////////////////////////////////
	CUIListItemServer* item = xr_new<CUIListItemServer>();
	int w = m_list.GetItemWidth();
	int h = m_list.GetItemHeight();
