...
void CActionPlanner<CScriptGameObject>::script_register(lua_State *L)
{
	...
	[
		class_<CScriptActionPlanner,CScriptActionPlannerWrapper>("action_planner")
			...
			.def(								constructor<>())
			.def("reinit",						&CScriptActionPlannerWrapper::reinit,	&CScriptActionPlannerWrapper::reinit_static)
			.def("update",						&CScriptActionPlannerWrapper::update,	&CScriptActionPlannerWrapper::update_static)
