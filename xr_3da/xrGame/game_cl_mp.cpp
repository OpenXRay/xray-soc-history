	...
	case 0:			//from hit
		{
			...
			if (pWeapon)
			{
				...
				if (pIItem)
				{
					...
					KMS.m_initiator.m_rect.add(pIItem->GetXPos()*INV_GRID_WIDTH, pIItem->GetYPos()*INV_GRID_HEIGHT);
					KMS.m_initiator.m_rect.add(pIItem->GetGridWidth()*INV_GRID_WIDTH, pIItem->GetGridHeight()*INV_GRID_HEIGHT);
