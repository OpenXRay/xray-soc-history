...
IC	void CALifeLevelRegistry::add				(CSE_ALifeDynamicObject *object)
{
	...
	if (psAI_Flags.test(aiALife)) {
		Msg				("[LSS] adding object [%s][%d] to current level",object->s_name_replace,object->ID);
	}
#endif
...
#ifdef DEBUG
	if (psAI_Flags.test(aiALife)) {
		Msg				("[LSS] removing object [%s][%d] from current level",object->s_name_replace,object->ID);
	}
#endif
