...
void CElevatorState::UpdateStClimbingDown()
{
	...
	if(!fis_zero(to_ax)&&!fis_zero(to_mag_and_dir(ca))&&abs(ca.dotproduct(d))<M_SQRT1_2)SwitchState(clbDepart);
	if(m_ladder->AxDistToLowerP(m_character)-m_character->FootRadius()<stop_climbing_dist)
		SwitchState(clbNearDown);
	
	if(m_ladder->DDToAxis(m_character,d)-m_character->FootRadius()>out_dist)SwitchState((clbNone));
}
void CElevatorState::UpdateStClimbingUp()
{
	...
	float toax=m_ladder->DDToAxis(m_character,d);
	if(toax-m_character->FootRadius()>out_dist)
											SwitchState((clbNone));
}
