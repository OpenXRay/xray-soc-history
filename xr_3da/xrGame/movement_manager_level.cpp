...
//				if (CLevelLocationSelector::used() && (CLevelLocationSelector::m_path->size() > 0))
//					set_level_dest_vertex( (*CLevelLocationSelector::m_path)[CLevelLocationSelector::m_path->size() - 1]);
		...
		/*******************************/
			...
			if (!CLevelPathManager::actual()) {
				...
				if (CDetailPathManager::completed(Position())) {
					...
					m_path_state	= ePathStateContinueLevelPath;
					Msg("* Path:: LevelPath size = [%u]", CLevelPathManager::path().size());
