...
//				}
	...
	StandUp();
	vfSetLookAndFireMovement(false, Leader != this ? WALK_FORWARD_4 : WALK_FORWARD_4,1.0f,Group,dwCurTime);
}
...
//				}
	...
	if (acosf(tWatchDirection.dotproduct(tTemp0)) < PI_DIV_2) {
		...
		if (fDistance >= m_fMaxPatrolDistance) {
			vfSetLookAndFireMovement(false, WALK_FORWARD_4,1.1f,Group,dwCurTime);
		}
		else
			if (fDistance <= m_fMinPatrolDistance) {
				vfSetLookAndFireMovement(false, WALK_FORWARD_4,0.9f,Group,dwCurTime);
			}
			else {
				vfSetLookAndFireMovement(false, WALK_FORWARD_4,float(((fDistance - (m_fMaxPatrolDistance + m_fMinPatrolDistance)*.5f)/((m_fMaxPatrolDistance - m_fMinPatrolDistance)*.5f)*.1f + m_fMinPatrolDistance)),Group,dwCurTime);
			}
	}
	else {
