...
void CAI_Soldier::SelectAnimation(const Fvector& _view, const Fvector& _move, float speed)
{
	...
	if (iHealth <= 0) {
		if (m_bCrouched)
			S = tSoldierAnimations.tCrouch.tGlobal.tpDeath;
		else {
			for (int i=0 ;i<2; i++)
				if (tSoldierAnimations.tNormal.tGlobal.tpaDeath[i] == m_current) {
					S = m_current;
					break;
				}
			if (!S)
				S = tSoldierAnimations.tNormal.tGlobal.tpaDeath[::Random.randI(0,2)];
		}
	} 
	else {
		if (speed<0.2f) 
			if ((!(AI_Path.TravelPath.size()) || (eCurrentState != aiSoldierPatrolDetour))) {
				...
				if (r_torso_target.yaw - r_torso_current.yaw > PI/30.f)
					if (m_bCrouched)	
						S = tSoldierAnimations.tCrouch.tLegs.tpTurn;
					else
						S = tSoldierAnimations.tNormal.tLegs.tpTurn;
				else
					if (m_bCrouched)	
						S = tSoldierAnimations.Crouch.tLegs.tpIdle;
					else
						S = tSoldierAnimations.tNormal.tLegs.tpIdle;
			}
			else
				S = m_current;
		else {
	...
	if (S!=m_current) { 
		...
		m_current = S;
		if (S) {
			CBlend *tpBlend = PKinematics(pVisual)->PlayCycle(S);
			if (S == m_walk.fwd) {
