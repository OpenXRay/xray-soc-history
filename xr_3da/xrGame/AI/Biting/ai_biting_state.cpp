...
{
	// ≈сли путь построен выполнить SetDirectionLook 
...
//Info: если в классах-потомках, используютс€ дополнительные пол€ времени,
...
void CAttack::Run()
{
	...
	switch (m_tAction) {
		case ACTION_RUN:
			...
			SetNextThink(300);	// чем больше рассто€ние, тем больше SetNextThink
