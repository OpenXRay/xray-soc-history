...
BOOL CAI_Biting::net_Spawn (LPVOID DC) 
{
	...
	// ”становить новый Visual, перезагрузить анимации
	MotionMan.UpdateVisual();

	m_PhysicMovementControl->SetPosition	(Position());
	m_PhysicMovementControl->SetVelocity	(0,0,0);

	// ”становить алгоритм групповой атаки
...
void CAI_Biting::PitchCorrection() 
{
	...
	// находим проекцию точки, лежащей на векторе текущего направлени€
	Fvector dir_point, proj_point;
	dir_point.mad(Position(), Direction(), 1.f);
	P.project(proj_point,dir_point);
	
	// получаем искомый вектор направлени€
	Fvector target_dir;
