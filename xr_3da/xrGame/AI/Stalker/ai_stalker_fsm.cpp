...
void CAI_Stalker::Think()
{
	...
	int						iIndex;
	if (!K && _K && (((iIndex = ifFindDynamicObject(m_tSavedEnemy)) != -1) && (Level().timeServer() - m_tpaDynamicSounds[iIndex].dwTime < m_dwInertion)) && m_tpaDynamicSounds[iIndex].tpEntity->g_Alive()) {
		K = true;
		C = _C;
		E = _E;
		D = _D;
