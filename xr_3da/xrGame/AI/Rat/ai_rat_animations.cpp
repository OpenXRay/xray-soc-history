...
void CAI_Rat::SelectAnimation(const Fvector& _view, const Fvector& _move, float speed)
{
	...
	if (iHealth <= 0) {
		...
		if (!tpGlobalAnimation)
			tpGlobalAnimation = tRatAnimations.tNormal.tGlobal.tpaDeath[::Random.randIs(0,2)];
	}
	else
		if (m_bFiring) {
			for (int i=0 ;i<3; i++)
			...
			if (!tpGlobalAnimation || !m_tpCurrentGlobalBlend || !m_tpCurrentGlobalBlend->playing)
				tpGlobalAnimation = tRatAnimations.tNormal.tGlobal.tpaAttack[::Random.randIs(0,3)];
