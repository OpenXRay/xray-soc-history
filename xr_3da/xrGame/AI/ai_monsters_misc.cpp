...
u32 dwfChooseAction(u32 dwActionRefreshRate, float fMinProbability0, float fMinProbability1, float fMinProbability2, float fMinProbability3, u32 dwTeam, u32 dwSquad, u32 dwGroup, u32 a0, u32 a1, u32 a2, u32 a3, u32 a4, CEntity *tpEntity, float fGroupDistance)
{
	...
	WRITE_QUERY_TO_LOG("\nNew query");
	if (bfGetActionSuccessProbability(Members,VisibleEnemies,fMinProbability0,*getAI().m_pfVictoryProbability)) {
		...
		if (bfGetActionSuccessProbability(Members,VisibleEnemies,fMinProbability1,*getAI().m_pfVictoryProbability)) {
			Group.m_dwLastActionTime	= Level().timeServer();
			...
			if (bfGetActionSuccessProbability(Members,VisibleEnemies,fMinProbability2,*getAI().m_pfVictoryProbability)) {
				Group.m_dwLastActionTime	= Level().timeServer();
				...
				if (bfGetActionSuccessProbability(Members,VisibleEnemies,fMinProbability3,*getAI().m_pfVictoryProbability)) {
					Group.m_dwLastActionTime	= Level().timeServer();
