...
//	MotionMan.AddReplacedAnim(&m_bDamaged, eAnimWalkFwd,	eAnimWalkDamaged);
	...
	if (MotionMan.start_load_shared(CLS_ID)) {
		...
		MotionMan.AddAnim(eAnimStandTurnLeft,	"stand_turn_left_",		-1, &inherited::get_sd()->m_fsVelocityStandTurn,		PS_STAND);
		MotionMan.AddAnim(eAnimStandTurnRight,	"stand_turn_right_",	-1, &inherited::get_sd()->m_fsVelocityStandTurn,		PS_STAND);
