...
void CAI_Space::q_Range(DWORD StartNode, const Fvector& BasePos, float Range, AI::NodeEstimator& Estimator, MemberNodes& MemberPlaces)
{
	if (0==vfs)	return;
	...
	// Cycle
	for (int i=0; i<MemberPlaces.size(); i++)
		q_mark[MemberPlaces[i]] = true;
	...
	// Clear q_marks
	for ( i=0; i<MemberPlaces.size(); i++)
		q_mark[MemberPlaces[i]] = false;
...
void CAI_Space::vfSearchNodeInTheDirection(DWORD StartNode, const Fvector& BasePos, float Range, AI::NodeEstimator& Estimator, MemberNodes& MemberPlaces, Fvector tSearchDirection)
{
	if (0==vfs)	return;
