...
void CActor::Hit		(float iLost, Fvector &dir, CObject* who, s16 element, float impulse)
{
	...
	if(pCreator->CurrentEntity() == this) {
		...
		Level().Cameras.AddEffector(xr_new<CEffectorPPHit>(svTransform.i.dotproduct(l_d), svTransform.j.dotproduct(l_d), .001f*iLost, .003f*iLost));
		Level().Cameras.AddEffector(xr_new<CEffectorHit>(svTransform.i.dotproduct(l_d), svTransform.j.dotproduct(l_d), .001f*iLost, .003f*iLost));
	}
	switch (GameID())
	{
	case GAME_SINGLE:		
