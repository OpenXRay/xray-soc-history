...
void CSoundMemoryManager::add_sound_object(CObject *object, int sound_type, const Fvector &position, float sound_power)
{
	...
	if (visual_memory_manager) {
		xr_vector<CVisibleObject>::const_iterator I = std::find(visual_memory_manager->memory_visible_objects().begin(),visual_memory_manager->memory_visible_objects().end(),entity_alive->ID());
	...
	xr_vector<CSoundObject>::iterator	J = std::find(m_sounds->begin(),m_sounds->end(),game_object->ID());
	if (m_sounds->end() == J) {
