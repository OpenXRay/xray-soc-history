...
public:
	...
	virtual			~CAStar();
			...
			float	ffFindMinimalPath(u32 dwStartNode, u32 dwGoalNode, NodePath& Result, bool bUseMarks = false);
			float	ffFindOptimalPath(u32 dwStartNode, u32 dwGoalNode, NodePath& Result, float fLightWeight = DEFAULT_LIGHT_WEIGHT, float fCoverWeight = DEFAULT_COVER_WEIGHT, float fDistanceWeight = DEFAULT_DISTANCE_WEIGHT, bool bUseMarks = false);
			float	ffFindOptimalPath(u32 dwStartNode, u32 dwGoalNode, NodePath& Result, u32 dwEnemyNode, float fOptimalEnemyDistance, float fLightWeight = DEFAULT_LIGHT_WEIGHT, float fCoverWeight = DEFAULT_COVER_WEIGHT, float fDistanceWeight = DEFAULT_DISTANCE_WEIGHT, float fEnemyViewWeight = DEFAULT_ENEMY_VIEW_WEIGHT, bool bUseMarks = false);
			float	ffFindOptimalPath(u32 dwStartNode, u32 dwGoalNode, NodePath& Result, Fvector tEnemyPosition, float fOptimalEnemyDistance, float fLightWeight, float fCoverWeight, float fDistanceWeight, float fEnemyViewWeight, bool bUseMarks = false);
};
