////////////////////////////////////////////////////////////////////////////
//	Module 		: alife_human_patrol_path_manager.cpp
//	Created 	: 01.11.2005
//  Modified 	: 01.11.2005
...
CALifeHumanPatrolPathManager::CALifeHumanPatrolPathManager	(object_type *object)
{
	VERIFY					(object);
	m_object				= object;
...
void CALifeHumanPatrolPathManager::path						(const shared_str &path_name)
{
	path					(ai().patrol_paths().path(path_name));
}

const CALifeHumanPatrolPathManager::_GRAPH_ID &CALifeHumanPatrolPathManager::target_game_vertex_id	() const
{
	return								(
		path().vertex(
...
const u32 &CALifeHumanPatrolPathManager::target_level_vertex_id	() const
{
	return								(
		path().vertex(
			m_current_vertex_index
...
void CALifeHumanPatrolPathManager::actualize				()
{
	m_current_vertex_index				= u32(-1);
	m_previous_vertex_index				= u32(-1);
	m_actual							= true;
...
bool CALifeHumanPatrolPathManager::location_reached			() const
{
...
void CALifeHumanPatrolPathManager::navigate					()
{
...
void CALifeHumanPatrolPathManager::update					()
{
	if (!m_path)
		return;
