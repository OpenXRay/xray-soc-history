...
public:
	...
	IC		bool	inside						(const CVertex *vertex, const CLevelGraph::CPosition &vertex_position) const;
	IC		bool	inside						(const CVertex *vertex, const Fvector &position) const;
	...
	IC		float	cover_in_direction			(float angle, u32 vertex_id) const;
			...
			void	find_game_point_in_direction(u32 start_vertex_id, const Fvector &start_point, const Fvector &tDirection, u32 &finish_vertex_id, _GRAPH_ID tGraphID) const;
