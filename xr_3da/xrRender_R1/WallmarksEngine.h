...
private:
	...
	void		AddWallmark_internal(const Fvector &contact_point, ref_shader hTexture, float sz);

	wallmark*	wm_allocate			(ref_shader&	S	);
	void		wm_render			(wallmark*	W, FVF::LIT* &V);
	void		wm_destroy			(wallmark*	W	);
public:
	void		AddWallmark			(const Fvector &contact_point, ref_shader hTexture, float sz);
	void		Render				();
