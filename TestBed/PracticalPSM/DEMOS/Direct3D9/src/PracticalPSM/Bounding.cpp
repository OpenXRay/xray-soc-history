...
BoundingCone::BoundingCone(const std::vector<BoundingBox>* boxes, const D3DXMATRIX* projection, const D3DXVECTOR3* _apex) : apex(*_apex)
{
    ...
    default:
    {
        ...
        //  compute an optimized direction vector by finding the two points with maximal angle between them, and use the
        //  halfway vector as the cone direction.
        //  not perfectly minimal, but decent.

        //  start by xforming all points to post-projective space, and build normalized direction vectors for each
        ...
        D3DXVECTOR3 *a=NULL, *b=NULL;

        const unsigned int nSize = (unsigned int)ppPts.size();

        direction = D3DXVECTOR3(0,0,0);
        for (i=0; i<nSize; i++)
        {
            D3DXVECTOR3 tmp = ppPts[i] - apex;
            tmp -= apex;
...
void GetSceneBoundingBox( BoundingBox* box, const NVBScene* scene )
{
    BoundingBox tmp;
