	...
	if((g_dwOSVerFlags & dwWin95) == dwWin95)
	{
		if((dwReason & dwDCOM) == dwDCOM)
		{
			...
			else
			{
				...
				{
					// at root directory
					wsprintf(lpRunOnceCmd, TEXT("%s%s %s"), szCurrentDir, g_szAppName, lpDCOMVal);
				}
				else
					wsprintf(lpRunOnceCmd, TEXT("%s\\%s %s"), szCurrentDir, g_szAppName, lpDCOMVal);
#ifdef _DEBUG
				MessageBox(NULL, lpRunOnceCmd, TEXT("DCOM RunOnce Reg Entry"), MB_OK);
#endif
/*
...
*/
		...
		if((dwReason & dwMDAC) == dwMDAC)
		{
			...
			else
			{
				...
				{
					// at root directory
					wsprintf(lpRunOnceCmd, TEXT("%s%s %s"), szCurrentDir, g_szAppName, lpMDACVal);
				}
				else
					wsprintf(lpRunOnceCmd, TEXT("%s\\%s %s"), szCurrentDir, g_szAppName, lpMDACVal);
#ifdef _DEBUG
				MessageBox(NULL, lpRunOnceCmd, TEXT("MDAC RunOnce Reg entry"), MB_OK);
#endif
