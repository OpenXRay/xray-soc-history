...
BOOL InstallDCOM()
{
	...
	{
		// at root directory
		wsprintf(szFileName, TEXT("%sDCOM95.EXE /Q"), szCurrentDir);
	}
	else
		wsprintf(szFileName, TEXT("\"%s\\%s\" /Q"), szCurrentDir, TEXT("DCOM95.EXE"));
#ifdef _DEBUG
	MessageBox(NULL, szFileName, TEXT("DCOM Filename"), MB_OK);
#endif
	// old way of starting up DCOM didn't account for whitespace or root directory
