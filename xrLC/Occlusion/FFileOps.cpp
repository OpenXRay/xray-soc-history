...
void Fatal(char *e, char *d) {
	char buf[256];
	sprintf(buf,"! FATAL ERROR: %s, %s",e,d);
	Log(buf);
	THROW;
}

_EF(void *	LoadPAK(char *name, DWORD *pdwSize))
...
_EF(bool InPAK(char *name))
	return (bsearch(name, cacheHDR, cacheCNT, sizeof(_file),_compare)!=NULL);
_end;

_EF(ENGINE_API void *DownloadFile(char *fn, DWORD *pdwSize))
{
	int		hFile;
	DWORD	size;
...
_EF(ENGINE_API bool FileExists(char *fn))
	int		hFile;
