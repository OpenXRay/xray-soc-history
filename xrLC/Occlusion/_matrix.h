...
public:
	...
	_E(__forceinline	void	invert(const _matrix & a ))			// faster than self-invert
		identity();
	...
	_E(__forceinline	void	div( const _matrix &A, float v ))
		VERIFY(fabsf(v)>0.000001f);
		mul(A,1.0f/v);
	_end;
	_E(__forceinline	void	div( float v ))
		VERIFY(fabsf(v)>0.000001f);
		mul(1.0f/v);
	_end;
	_E(__forceinline	void	build_projection(float fFOV, float fAspect, float fNearPlane, float fFarPlane))
		...
		_43 = -Q*fNearPlane;
	_end;
	_E(__forceinline	void	build_camera(Fvector &vFrom, Fvector &vAt, Fvector &vWorldUp))
